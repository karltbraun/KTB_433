...
def get_protocol_description(proto_id: int) -> str:
    return dct_protocols.get(proto_id, "ID Not Found")


def dump_protocol_list() -> None:
    for protocol in dct_protocols:
        print(f"{protocol}: {dct_protocols[protocol]}")


def test_get_protocol_description(n: int = 10) -> None:
    # create a list of n random protocol ids
    n = 10
    lst = [randint(1, 256) for i in range(n)]

    # print the description of each protocol
    for proto in lst:
        print(f"{proto}: {get_protocol_description(proto)}")


def get_protocol_descriptions(proto_ids: list) -> dict:
    return {proto_id: get_protocol_description(proto_id) for proto_id in proto_ids}
...
def main() -> int:
    ...
    test_get_protocol_description(20)
